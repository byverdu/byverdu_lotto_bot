...
const { scrapper, utils: { logErrorBuilder } } = require('./lib')
...
(async () => {
  try {
    ...
    groupId = botUpdate.length === 0 ?
      ...
      botUpdate.find(items => items.message.chat.type === 'group').message.chat.id;
  ...
  .then(() => {
    scrapper({ debug, headless: JSON.parse(HEADLESS_CHROME), luckyNumbers })
      .then(() => {
        ...
        bot.sendPhoto(groupId, './lotto_results.png')
          .then(resp => {
